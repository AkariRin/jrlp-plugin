...
你可以通过修改配置文件中的 `command.regex` 来自定义触发命令的正则表达式：

```toml
[command]
# 示例1: 添加更多触发词
regex = '^(今日老婆|抽老婆|jrlp|今天老婆|我的老婆|抽个老婆)$'

# 示例2: 支持带前缀的命令（如 /、!、！）
regex = '^[/!！](今日老婆|抽老婆|jrlp)$'

# 示例3: 支持更灵活的匹配（包含"老婆"关键词即可触发）
regex = '.*老婆.*'
```

> ⚠️ **重要提示**: 
> - 修改正则表达式后，需要**重载插件**才能生效：`/pm plugin reload jrlp-plugin`
> - 请确保正则表达式格式正确，错误的正则可能导致命令无法触发
> - 建议先在正则测试工具中验证表达式
