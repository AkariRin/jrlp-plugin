...
def get_group_member_list(address: str, port: int, group_id: str) -> Tuple[bool, Union[list, str]]:
    ...
    success, result = _make_request(url, payload)
    if not success:
        return False, result
...
def get_member_info(address: str, port: int, group_id: str, user_id: str) -> Tuple[bool, Union[dict, str]]:
    """获取单个群成员信息
...
def build_today_wife_message(user_id: str, wife_id: str, wife_nickname: str) -> list:
    """构建今日老婆消息"""
    return [
        {"type": "at", "data": {"qq": user_id}},
        {"type": "text", "data": {"text": "\n你今天的群老婆是:"}},
        {"type": "image", "data": {"file": f"https://q1.qlogo.cn/g?b=qq&nk={wife_id}&s=640", "summary": "[图片]"}},
        {"type": "text", "data": {"text": f"{wife_nickname}({wife_id})"}}
    ]


def build_already_obtained_message(user_id: str, wife_id: str, wife_nickname: str) -> list:
    """构建已抽取过老婆的提示消息"""
    return [
        {"type": "at", "data": {"qq": user_id}},
        {"type": "text", "data": {"text": "\n你今天已经有群老婆了，要好好对待她哦~"}},
...
class JrlpCommand(BaseCommand):
    ...
    async def execute(self) -> Tuple[bool, Optional[str], bool]:
        ...
        if existing_wife:
            # 已抽取过，获取老婆信息并返回
            success, wife_info = get_member_info(napcat_address, napcat_port, group_id, existing_wife)
            if not success:
            ...
            message = build_already_obtained_message(user_id, existing_wife, wife_nickname)

            success, error = send_group_message(napcat_address, napcat_port, group_id, message)
            if not success:
        ...
        # 获取群成员列表
        success, member_list = get_group_member_list(napcat_address, napcat_port, group_id)
        if not success:
        ...
        # 获取群信息
        success, group_info = get_group_info(napcat_address, napcat_port, group_id)
        if success:
        ...
        # 发送消息
        message = build_today_wife_message(user_id, wife_id, wife_nickname)
        success, error = send_group_message(napcat_address, napcat_port, group_id, message)
        if not success:
            logger.error(f"发送消息失败: {error}")
            return False, f"发送消息失败: {error}", True
